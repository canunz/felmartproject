...
module.exports = {
  ...
  mostrarAdmin: async (req, res) => {
    const precios = await PrecioResiduo.findAll({ where: { activo: 1 } });
  ...
  cotizarAvanzado: async (req, res) => {
    try {
      ...
      const todosLosResiduos = await PrecioResiduo.findAll();
      console.log('Todos los residuos en BD:', todosLosResiduos.map(r => ({
        id: r.id,
      ...
      // Log de depuración
      const residuosBD = await PrecioResiduo.findAll({ where: { activo: 1 } });
      console.log('=== DEPURACIÓN ===');
      console.log('Residuos activos en BD:', residuosBD.map(x => ({
        id: x.id,
      ...
      for (let r of residuos) {
        console.log('\nBuscando residuo:', {
          descripcion: r.descripcion,
          descripcionLower: r.descripcion.toLowerCase()
        });

        // Búsqueda exacta primero
        const residuoExacto = await PrecioResiduo.findOne({
          where: {
            descripcion: r.descripcion,
            activo: 1
          }
        ...
        console.log('Búsqueda exacta:', residuoExacto ? 'Encontrado' : 'No encontrado');

        // Si no se encuentra exacto, buscar con LIKE
        const residuoBD = residuoExacto || await PrecioResiduo.findOne({
          where: {
            [Op.and]: [
              where(fn('LOWER', col('descripcion')), 'LIKE', `%${r.descripcion.toLowerCase()}%`),
              { activo: 1 }
            ]
          }
        });

        console.log('Resultado final de búsqueda:', residuoBD ? {
          id: residuoBD.id,
          descripcion: residuoBD.descripcion,
          precio: residuoBD.precio,
          activo: residuoBD.activo
        } : 'No encontrado');

        if (!residuoBD) {
          return res.status(400).render('error', {
            titulo: 'Error',
            mensaje: `El residuo "${r.descripcion}" no tiene precio válido en la base de datos. Por favor revisa los precios de los residuos.`
          });
        }
        let valorUF = null;
        let precioUnitario = Number(residuoBD.precio);
        let subtotal = 0;
        if (residuoBD.moneda === 'UF') {
          try {
            valorUF = await require('../services/cmfBancosService').obtenerValorUF();
          } catch (e) {
            valorUF = await require('../models/PrecioResiduo').obtenerValorUF();
          }
          precioUnitario = valorUF * Number(residuoBD.precio);
        }
        subtotal = precioUnitario * Number(r.cantidad);
        // Validar que los valores sean numéricos
        if (isNaN(precioUnitario) || isNaN(subtotal)) {
          return res.status(400).render('error', {
            titulo: 'Error',
            mensaje: `El residuo "${r.descripcion}" no tiene precio válido. Por favor revisa los precios de los residuos.`
          });
        }
        total += subtotal;
        detalles.push({
          ...
          subtotal: subtotal,
          valorUF: residuoBD.moneda === 'UF' ? valorUF : null
        });
      }
  ...
  calcularCotizacionAvanzada: async (req, res) => {
    try {
      ...
      let total = 0;
      let detalles = [];
      let errorResiduos = null;
      for (let r of residuos) {
        // Buscar el residuo en la base de datos
        const residuoBD = await require('../models/PrecioResiduo').findOne({
          where: {
            [Op.and]: [
              where(fn('LOWER', col('descripcion')), 'LIKE', `%${r.descripcion.toLowerCase()}%`),
              { activo: 1 }
        ...
        if (!residuoBD) {
          errorResiduos = `El residuo "${r.descripcion}" no tiene precio válido en la base de datos. Por favor revisa los precios de los residuos.`;
          break;
        }
        let valorUF = null;
        let precioUnitario = Number(residuoBD.precio);
        ...
        subtotal = precioUnitario * Number(r.cantidad);
        if (isNaN(precioUnitario) || isNaN(subtotal)) {
          errorResiduos = `El residuo "${r.descripcion}" no tiene precio válido. Por favor revisa los precios de los residuos.`;
          break;
        }
        total += subtotal;
        detalles.push({
          ...r,
          unidad: residuoBD.unidad,
          moneda: residuoBD.moneda,
          precioUnitario: precioUnitario,
          subtotal: subtotal,
          valorUF: residuoBD.moneda === 'UF' ? valorUF : null
        });
      ...
      // Guardar la cotización principal con los detalles en JSON
      try {
        await require('../models/Cotizacion').create({
        ...
        console.error('Error al guardar cotización en BD:', e);
      ...
      // Renderizar la vista de resultado SIEMPRE
      res.render('cotizaciones/resultado', {
        detalles,
        total: totalFinal,
        neto,
